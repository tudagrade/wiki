...
Zum Erstellen der Abgabedatei wird mit der Gradle Task **"prepareSubmission** verwendet. Befolgen Sie dafür die Schritte auf dieser Seite.  
   * Beachten Sie, dass ihre Code zum Abgeben kompilierbar sein muss. IntelliJ zeigt Ihnen am unteren Rand im Abschnitt **"Problems"** alle Syntaxfehler in rot an.
...
3. Unter **<ProjektName\> -> Task -> submit** finden Sie die Gradle Task **"prepareSubmission**. Führen Sie diese mit einem Doppelclick aus.
...
4. In dem Projektordner finden sie nun im Ordner **"build"** einen weiteren Ordner **"libs"**. In diesem befindet sich die Abgabedatei.
   * Die Datei ist nach dem Schema **"HXX-lastName-firstName-submission.jar"** benannt. Ändern Sie diese Bennung **nicht**.
